import { client, showsQuery } from "@/lib/sanity";
import { shows as staticShows } from "@/lib/data";

export const metadata = {
  title: "Past Shows – Muck Savage",
  description:
    "A history of Muck Savage performances across Colorado and Missouri.",
};

export default async function PastShowsPage() {
  const shows = await client.fetch(showsQuery).catch(() => null);
  const showList = shows?.length ? shows : staticShows;

  // Group shows by year
  const showsByYear: Record<number, any[]> = {};
  showList.forEach((show: any) => {
              ...
              {years.map((year) => (
                <div key={year} className="relative">
                  ...
                  <div className="space-y-3 pl-12 md:pl-20">
                    {showsByYear[year].map((show: any, i: number) => (
                      <div
                        key={`${show.venue}-${i}`}
